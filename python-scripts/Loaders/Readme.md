...
| New notebook name | Old name | Main role | Paper/repository connection |
|---|---|---|---|
| `make_density_gifs.ipynb` | `Loader_Gifs.ipynb` | Build PNG frames and GIFs from saved density snapshots `rho_prob_t*.npy`. | Repository media and representative density animations. |
| `plot_detection_time_distribution.ipynb` | `Loader_MainSolver.ipynb` | Reconstruct survival, roof-flux density, finite-window restricted mean, trajectory histogram overlay, and summary files. | Closest post-processing notebook for Letter Fig. 1 and Supplement S2--S4. |
| `plot_bohmian_trajectories.ipynb` | `Loader_Trajectories.ipynb` | Plot selected Pauli-current trajectories in 3D and as XY/XZ/YZ projections. | Trajectory visualization companion; in this project, trajectories are samples of the detector-present flux law. |
