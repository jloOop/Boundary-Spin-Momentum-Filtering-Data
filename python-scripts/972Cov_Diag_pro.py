...
# -*- coding: utf-8 -*-
"""
3-D Crank--Nicolson Schrödinger solver with boundary-layer diagnostics
for the spinor absorbing boundary condition.

The TDSE, Crank--Nicolson, GMRES, and spinor-ABC matrix implementation are
kept unchanged from the supplied solver.  The diagnostic layer is rewritten
for the new approach: Pi_- is not neglected.  The code evaluates the full
two-branch boundary-layer map exp(-epsilon J), the first-order covariance
coefficient beta_omega = Cov_{nu0}(t, s a_omega), exact finite-epsilon
checks, RMST coefficients, and full two-branch Duhamel/slab remainder checks.
"""
...
run_name = (
    f"3052.BoundaryLayerCovariance_spinorABC"
